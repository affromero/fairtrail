...
export function validateCarRunView(raw: unknown, expectedId: string, standalone = false, now = new Date()) {
  ...
  if (id !== expectedId) throw new CarError('Rental status belongs to another search');
  ...
  return { id, trackerId, status, createdAt, completedAt, search, result, error: value.error === null ? null : carText(value.error, 16000, 'search error') };
}
