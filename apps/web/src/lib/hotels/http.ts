...
import { wakeTravelWorker } from '../travel/http';
import { TravelJobError } from '../travel/jobs';

export function wakeHotelWorker() { wakeTravelWorker(); }
export async function hotelEndpoint(action: (actor: HotelActor) => Promise<Response>): Promise<Response> {
  try { return await action(await hotelActor()); }
    ...
    if (error instanceof HotelError || error instanceof TravelJobError) return apiError(error.message, error.status);
