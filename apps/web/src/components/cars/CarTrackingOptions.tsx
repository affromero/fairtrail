...
export function CarTrackingOptions({ value, currency, disabled, invalid, onChange }: { value: CarOptionsDraft; currency: string; disabled: boolean; invalid: boolean; onChange: (value: CarOptionsDraft) => void }) {
  ...
  return <fieldset className={styles.options} disabled={disabled} aria-describedby={`${id}-help${invalid ? ` ${id}-error` : ''}`}>
    ...
    <div className={styles.fields}>
      <label className={styles.field}>{t('mode')}<select value={value.mode} onChange={e => onChange({ ...value, mode: e.target.value as CarOptionsDraft['mode'] })}><option value="best">{t('best')}</option><option value="contract">{t('contract')}</option></select></label>
